...
parser.add_argument('--config', type=str, help="yaml configuration file", default='./tests/models/configs/cylinder_2d.yaml')
...
if __name__ == '__main__':
    ...
    data_weighting = params.get('data_loss_weight',1)
    pde_weighting = params.get('pde_loss_weight',0)
    optimizer = torch.optim.Adam(model.parameters(), params['learning_rate'])
    scheduler = schedulers.OneCycleLR(optimizer, max_lr=params['init_lr'], 
    ...
    for epoch in range(params['epochs']):
        ...
        for batch in train_dataloader:
            ...
            training_loss = data_loss * data_weighting
            training_loss += torch.stack(pde_losses).mean() * pde_weighting
